...
print('BUY prob:' , round(info['probabilities']['BUY']*100, 1), '%')
print('SELL prob:', round(info['probabilities']['SELL']*100, 1), '%')
print('HOLD prob:', round(info['probabilities']['HOLD']*100, 1), '%')

proba = model.predict_proba(bundle.X_test)
print('Max confidence in test:', round(np.max(proba)*100, 1), '%')
